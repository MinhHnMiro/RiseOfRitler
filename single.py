...
title = font.render('Rise Of Ritler', False, (255,255,255))
title_size = (title.get_width(), title.get_height())
    ...
    if state == 'menu':
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONUP:
                pass
        size = wn.get_size()
        coords = (size[0]//2 - title_size[0]//2, size[1]-size[1]*0.9)
        wn.blit(title, coords)
        pygame.display.update()
        clock.tick(FPS)
        wn.fill('black')
